...
impl HierarchicalCache {
    ...
    pub fn clear_clips(&mut self) {
        let mut cache = self.clip_cache.blocking_write();
        cache.clear();
    }

    /// 清空VAE缓存
    pub fn clear_vaes(&mut self) {
        let mut cache = self.vae_cache.blocking_write();
        cache.clear();
    }
    ...
    pub fn get_model_state(&self, model_id: &str) -> Option<bool> {
        let cache = self.model_cache.blocking_read();
    ...
    pub fn get_clip_state(&self, clip_id: &str) -> Option<bool> {
        let cache = self.clip_cache.blocking_read();
    ...
    pub fn get_vae_state(&self, vae_id: &str) -> Option<bool> {
        let cache = self.vae_cache.blocking_read();
